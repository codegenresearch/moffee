...
def test_paginate_creates_correct_number_of_pages(sample_document: str):
    pages: List[Page] = composite(sample_document)
...
def test_page_splitting_on_dividers_with_different_characters():
    doc: str = """\nContent 1\n---\nContent 2\n===\nContent 3\n---\nContent 4\n---\nContent 5\n"""
    pages: List[Page] = composite(doc)
    assert len(pages) == 5


def test_page_splitting_on_dividers_with_horizontal_character():
    doc: str = """\nContent 1\n---\nContent 2\n---\nContent 3\n---\nContent 4\n"""
    pages: List[Page] = composite(doc)
    assert len(pages) == 4
...
def test_chunking_hybrid():
    ...
    next_chunk: Chunk = chunk.children[1]
