...
![Image-2](image2.png)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        doc_path: str = os.path.join(temp_dir, "test.md")
        res_dir: str = os.path.join(temp_dir, "resources")
...
def test_rendering(setup_test_env: tuple) -> None:
    _, doc_path, _, _ = setup_test_env
    with open(doc_path, encoding="utf8") as f:
        doc: str = f.read()
    html: str = render_jinja2(doc, template_dir())
...
def test_read_options(setup_test_env: tuple) -> None:
    _, doc_path, _, _ = setup_test_env
...
def test_build(setup_test_env: tuple) -> None:
    temp_dir: str
    doc_path: str
    res_dir: str
    output_dir: str
    ...
    asset_dir: list = os.listdir(j(output_dir, "assets"))
