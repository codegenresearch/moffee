...
def test_empty_deco():
    line = "@()"
    option = parse_deco(line)
    assert option == PageOption(width=800, height=600, aspect_ratio=800 / 600)
...
def test_deco_with_base_option():
    ...
    base_option = PageOption(
        layout="content", default_h1=False, default_h2=True, default_h3=True, width=800, height=600
    )
...
def test_deco_with_type_conversion():
    line = "@(default_h1=true, default_h2=false, layout=centered, custom_int=42, custom_float=3.14, width=1280, height=720)"
    base_option = PageOption(width=800, height=600)
    updated_option = parse_deco(line, base_option)
