...
class PageOption:
    ...
    aspect_ratio: Tuple[int, int] = (16, 9)  # Default aspect ratio
...
class Page:
    ...
    def _calculate_dimensions(self):
        """\n        Calculate dimensions based on the aspect ratio.\n        Handles aspect ratio errors gracefully.\n        """
        try:
            width, height = self.option.aspect_ratio
            if width <= 0 or height <= 0:
                raise ValueError("Aspect ratio dimensions must be positive integers.")
            self.dimensions = (1920, int(1920 * (height / width)))  # Example base width
        except ValueError as e:
            print(f"Error calculating dimensions: {e}")
            self.dimensions = (1920, 1080)  # Fallback to 16:9
