...
def composite(document: str) -> List[Page]:
    ...
    # Create the last page if there's remaining content\n    create_page()\n\n    # Process each page and choose titles\n    env_h1 = env_h2 = env_h3 = None\n    for page in pages:\n        inherit_h1 = page.option.default_h1\n        inherit_h2 = page.option.default_h2\n        inherit_h3 = page.option.default_h3\n        if page.h1 is not None:\n            env_h1 = page.h1\n            env_h2 = env_h3 = None\n            inherit_h1 = inherit_h2 = inherit_h3 = False\n        if page.h2 is not None:\n            env_h2 = page.h2\n            env_h3 = None\n            inherit_h2 = inherit_h3 = False\n        if page.h3 is not None:\n            env_h3 = page.h3\n            inherit_h3 = False\n        if inherit_h1:\n            page.h1 = env_h1\n        if inherit_h2:\n            page.h2 = env_h2\n        if inherit_h3:\n            page.h3 = env_h3\n\n    return pages\n\n\n# Enhanced test coverage for Markdown helper functions\ndef test_get_header_level():\n    assert get_header_level("# Header") == 1\n    assert get_header_level("## Header") == 2\n    assert get_header_level("### Header") == 3\n    assert get_header_level("#### Header") == 4\n    assert get_header_level("##### Header") == 5\n    assert get_header_level("###### Header") == 6\n    assert get_header_level("Not a header") == 0\n\n\ndef test_is_divider():\n    assert is_divider("---", "-")\n    assert is_divider("___", "_")\n    assert is_divider("***", "*")\n    assert not is_divider("----", "-")\n    assert not is_divider("__", "_")\n    assert not is_divider("**", "*")\n    assert not is_divider("Header", "-")\n\n\ndef test_is_empty():\n    assert is_empty("")\n    assert is_empty(" ")\n    assert is_empty("\t")\n    assert is_empty("\n")\n    assert not is_empty("Not empty")\n    assert not is_empty("   Content   ")\n\n\ndef test_rm_comments():\n    assert rm_comments("<!-- Comment -->") == ""\n    assert rm_comments("<!-- Comment\nAnother comment -->") == ""\n    assert rm_comments("Text <!-- Comment --> More text") == "Text  More text"\n    assert rm_comments("Text <!-- Comment -->") == "Text "\n    assert rm_comments("Text") == "Text"\n\n\ndef test_contains_deco():\n    assert contains_deco("@(key=value)")\n    assert not contains_deco("No deco here")\n    assert contains_deco("   @(key=value)   ")\n    assert contains_deco("@(key1=value1, key2=value2)")\n\n\ndef test_parse_deco():\n    option = parse_deco("@(background-color=blue)")\n    assert option.styles == {"background-color": "blue"}\n\n    option = parse_deco("@(default_h1=false)")\n    assert option.default_h1 is False\n\n    option = parse_deco("@(background=blue, default_h1=false)")\n    assert option.default_h1 is False\n    assert option.styles == {"background": "blue"}\n\n\ndef test_parse_value():\n    assert parse_value("true") is True\n    assert parse_value("false") is False\n    assert parse_value("123") == 123\n    assert parse_value("123.45") == 123.45\n    assert parse_value("value") == "value"