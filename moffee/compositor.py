...
class Page:
    ...
    def validate_aspect_ratio(self):
        """\n        Validates the aspect ratio of the slide based on the provided dimensions.\n        Raises ValueError if the aspect ratio is not supported.\n        """
        supported_aspect_ratios = [(16, 9), (4, 3), (1, 1)]
        width, height = self.option.slide_width, self.option.slide_height
        gcd = math.gcd(width, height)
        aspect_ratio = (width // gcd, height // gcd)

        if aspect_ratio not in supported_aspect_ratios:
            raise ValueError(
                f"Unsupported aspect ratio: {aspect_ratio}. Supported ratios are {supported_aspect_ratios}."
            )
